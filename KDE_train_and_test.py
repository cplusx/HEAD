...
save_path = f'KDE/{dataset}/Hessian_{hessian_dim}-LSCF_{LSCF_dim}-norm_{norm_feat}/{kernel}_{bandwidth}_{adv_type}.txt'
os.makedirs(os.path.dirname(save_path), exist_ok=True)
if os.path.exists(save_path):
    exit()

adv_types = ['PGD10', 'FGSM', 'BIM', 'CW', 'DeepFool', 'AutoAttack', 'OnePixel', 'SparseFool']
net = 'vgg16'
...
def load_data(
    ...
    LSCF_feature = None
    if LSCF_dim > 0:
        file_path = f'precomputed_LSCF_feature/{dataset}/{net}_{adv_type}_{split}.pkl'
        with open(file_path, 'rb') as IN:
            res = pickle.load(IN)
...
train_benign = load_data(dataset=dataset, net=net, adv_type='benign', split='train', hessian_dim=hessian_dim, LSCF_dim=LSCF_dim, modulus_mode=modulus_mode)
test_benign = load_data(dataset=dataset, net=net, adv_type='benign', split='test', hessian_dim=hessian_dim, LSCF_dim=LSCF_dim, modulus_mode=modulus_mode)
test_adv = {
    'overall': []
}
for at in adv_types:
    test_adv[at] = load_data(dataset=dataset, net=net, adv_type=at, split='test', hessian_dim=hessian_dim, LSCF_dim=LSCF_dim, modulus_mode=modulus_mode)
    test_adv['overall'].append(test_adv[at])
test_adv['overall'] = np.concatenate(test_adv['overall'], axis=0)

# roc auc
def get_roc_from_data(KDE_model, inlier, outlier):
    y_score = kde.score_samples(np.concatenate([inlier, outlier]))
    min_val = -1e10
...
kde = KernelDensity(kernel=kernel, bandwidth=bandwidth).fit(train_benign)
inlier = test_benign
outlier = test_adv[adv_type]
_, _, auc = get_roc_from_data(kde, inlier, outlier)
